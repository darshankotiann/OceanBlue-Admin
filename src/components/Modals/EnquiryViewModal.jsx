...
const EnquiryViewModal = ({ setShowModal, showModal }) => {
    ...
    const assignOrder = (e) => {
        ...
        axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/order/add`,
            {
                vendorID: showModal?.data?.vendorID?._id,
                enquiryID: showModal?.data?._id,
                productID: showModal?.data?.productID?._id
            },
            { headers: headers })
            .then((res) => {
                ...
                <div class="relative bg-white rounded-lg shadow ">
                    ...
                    <div class="p-4 md:p-5 grid grid-cols-2 gap-x-4 gap-y-2">
                        ...
                        <div className="w-full flex flex-col gap-2">
                            ...
                            <input value={showModal?.data?.productID?.name} readOnly class="w-full bg-gray-50  text-gray-900 text-sm rounded-lg block  p-4 border-2 " />
                        </div>
                        <div className="w-full flex flex-col gap-2">
                            <label class="block w-1/2 items-center text-gray-800   font-medium text-sm">Product Price</label>
                            <input value={showModal?.data?.productID?.price} readOnly class="w-full bg-gray-50  text-gray-900 text-sm rounded-lg block  p-4 border-2 " />
                        </div>
                        ...
                        <button type="button" disabled={showModal?.data?.completed} onClick={assignOrder} className="disabled:bg-[#4D70B6] col-span-2 text-center flex justify-center focus:outline-none text-white bg-[#0064FF] hover:bg-[#4D70B6] focus:ring-4  font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2">
                            {showModal?.data?.completed ? "Assigned" : "Assign to Vendor"}
                        </button>
                    </div>
                </div>
            </div>
